...
- 提案：4213（新记忆单元首条证据）
- 证据积累：69（已有记忆再次通过质量门）
- 晋升 active：15（≥2 次一致证据）
- 直接应用：3 条（采纳 3 / 拒绝 0）
- 退休：0（被质量门拒绝 ≥2 次，不可信）

## 2 记忆库状态（按类型 × 状态）

- phrase: active 691 · pending 9799 · retired 17
...
| Option A | r:display | 选项 A | 6 | 3 | 0 | force-reboot/inch-by-inch/incremental-rts |
| A <#0080ff>simple</color> line of text. | r:display | 一条<#0080ff>简单的</color>文本行。 | 4 | 3 | 0 | goodmorning/happy-cat-tavern |
| You have selected link <#ffff00> ID 01 | r:display | 您选择了链接 <#ffff00>，ID为01。 | 4 | 3 | 0 | goodmorning/happy-cat-tavern |
| You have selected link <#ffff00> ID 02 | r:display | 您选择了链接 <#ffff00>，ID为02。 | 4 | 3 | 0 | goodmorning/happy-cat-tavern |
