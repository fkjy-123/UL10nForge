...
**版本 0.13.1** · 面向 Unity 游戏的本地化工具：扫描游戏（外部文本 + 二进制资源）→ 深度 AI 翻译 → 人工审校 → 原格式写回。
...
| **翻译** | 开始（可停止续传）→ 质量门 → 安全写回 `<游戏目录>_汉化` → 写回成功后「开始游戏」一键启动汉化副本 |
...
3. **字符串级过滤**：路径/程序集名/哈希/着色器属性/Input System 绑定路径（`<Keyboard>/z`）与 interactions（`Press(behavior=2)`）/Timeline 轨道名（`Animation Track (1)`）/URP 后处理/字体名等引擎字符串黑名单 + 频次过滤；`Press E to open`、`Hold [F] to interact` 等交互提示强显示证据提升，并保护按键 token
4. **对象级引擎配置保护**：序列化含 Input System 绑定路径/interactions 的对象（InputActionAsset，动作名如 `Proceed`/`Interact` 是运行时按键查找键，翻译即输入失效——morfosigame 实证点击/F 全部无反应）、含 Timeline 轨道名/剪辑名的对象（翻译拆坏字符串结构）、共享资源中无 m_GameObject 的 ScriptableObject 短词对象（动画剪辑 displayName），整对象降级为键不翻译；场景（level）对话对象不受影响
5. **键名保护（Localization 表键绝不翻译）**：`ui_newGame` / `MENU_PLAY` / `phone_call_01` 等键风格标识符、语言代码、JSON 键字段一律剔除；单词式写法（`Settings` / `V-SYNC`）是任意语言的 UI 标签，正常翻译。写回期二次防护：历史误译的键条目也不会写回
6. **增量收敛**：规则升级后重扫自动清理旧库中已淘汰的噪音条目
...
- **0.13.1**：翻译模块修复——「待翻译」计数与翻译引擎同源（低置信度引擎消息留档不再虚高/永不减少，悬停提示可在文本审校查看）；提取器噪音修复（空值配置行 `nolog=` 不再作为可译文本、base64 编码 ZIP 包被正确剔除——此前每游戏恒现的失败条目来源）
- **0.13.0**：移除发布前启动冒烟（对用户多余）；写回按钮右侧新增「开始游戏」——写回验证通过后亮起，一键启动汉化副本直接进入游戏
- **0.12.0**：引擎配置对象保护（Input System / Timeline / 共享资源配置整对象跳过，修复 morfosigame 汉化后点击对话与 F 跳过全部无反应）
